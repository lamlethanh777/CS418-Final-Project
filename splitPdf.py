...
pdf_file = "raw/Sach-Nom-Cong-Giao-1995-012.pdf"
...
def resize_image_byFactor(image_path, scale_factor=2, output_path=None):
    """
    Phóng to hình ảnh bằng một tỉ lệ phóng đại (scale_factor).

    :param image_path: Đường dẫn hình ảnh gốc
    :param scale_factor: Tỉ lệ phóng đại (mặc định là 2 lần kích thước gốc)
    :param output_path: Đường dẫn để lưu hình ảnh mới (nếu không, ghi đè lên hình ảnh gốc)
    """
    with Image.open(image_path) as img:
...
def perform_ocr(image_path, output_file):
...
# Tách từng trang PDF thành hình ảnh và thực hiện OCR
def process_pdf(pdf_path, start_page, end_page, output_folder):
    ...
    for page_num in range(start_page - 1, end_page):
        ...
        # resize_image_byFactor(image_path, scale_factor=5)

        # OCR và lưu kết quả
        output_file = os.path.join(output_folder, f"page_{page_num + 1}.txt")
        perform_ocr(image_path, output_file)
