...
def write_to_excel(
    ...
    for page_x, page_y in book_page_ranges:
        # as this is 1-1 matching, we can just take the first element
        page_x = page_x[0]
        page_y = page_y[0]
        ...
        qn_words_string = load_single_qngu_page(
            f"{input_qngu_folder}/page_{page_x}.json"
        )

        aligned_nomLinesString, aligned_qnLinesString = Levenshtein_Align_Line(
            nom_words_string, qn_words_string, sino_similar_dict, qn_sino_dict
        )
        ...
        for (nom_char, nom_status), (qn_char, qn_status) in zip(
            aligned_nomLinesString, aligned_qnLinesString
        ):
            count_char += 1
            ...
            if index_ocr_word == length_ocr_line:
                ...
                worksheet.write(
                    row_index_global, 0, formatted_image_name, black_text_format
                )
                worksheet.write(row_index_global, 1, formatted_id, black_text_format)
                worksheet.write(
                    row_index_global,
                    2,
                    json.dumps(boxes[index_ocr_line]),
                    black_text_format,
                )
                ...
                if (index_ocr_line) >= len(nom_sentences) - 1:
                    ...
                    if length_ocr_line == 1:
                        ...
                        worksheet.write(
                            row_index_global, 4, qn_format_pairs[1], qn_format_pairs[0]
                        )
                    else:
                        worksheet.write_rich_string(
                            row_index_global, 3, *nom_format_pairs
                        )
                        worksheet.write_rich_string(
                            row_index_global, 4, *qn_format_pairs
                        )
                ...
                if length_ocr_line == 1:
                    worksheet.write(
                        row_index_global, 3, nom_format_pairs[1], nom_format_pairs[0]
                    )
                    worksheet.write(
                        row_index_global, 4, qn_format_pairs[1], qn_format_pairs[0]
                    )
...
def processAlignment(
    ...
    page_ranges_file = "page_ranges_old.json"
